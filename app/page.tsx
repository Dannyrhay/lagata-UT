...
export default function Home() {
  ...
  return <main>
    ...
    {showMobileMenu && !isViewer && <div className="mobileActionMenu"><button onClick={() => { setShowPwaStatus(true); setShowMobileMenu(false); }}><span>●</span>App status centre</button><button onClick={() => { setTheme(theme === "light" ? "dark" : "light"); setShowMobileMenu(false); }}><span>{theme === "light" ? "◐" : "☀"}</span>{theme === "light" ? "Dark mode" : "Light mode"}</button><button onClick={() => openSetup()}><span>⚙</span>Manage tournament</button>{data.history[0]?.snapshot && <button onClick={() => { undoLast(); setShowMobileMenu(false); }}><span>↶</span>Undo latest change</button>}</div>}
