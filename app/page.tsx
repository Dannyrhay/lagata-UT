...
type Tournament = { name: string; format: Format; homeAndAway: boolean; players: Player[]; matches: Match[] };
...
const initial: Tournament = { name: "Friday Night League", format: "league", homeAndAway: false, players: demoPlayers, matches: initialMatches };
...
export default function Home() {
  ...
  const [shareState, setShareState] = useState<"idle" | "saving" | "copied" | "error">("idle"); const cloudLoaded = useRef(false); const creatingCloud = useRef(false);
  ...
  useEffect(() => {
    ...
    if (activeId) {
      const token = localStorage.getItem(`lagata-edit-${activeId}`) || "";
      setShareId(activeId); setEditToken(token);
      fetch(`${API_BASE}/api/tournament?id=${encodeURIComponent(activeId)}`).then((r) => r.ok ? r.json() : Promise.reject()).then(({ tournament }) => { setData({ ...tournament, format: tournament.format || "league", homeAndAway: Boolean(tournament.homeAndAway) }); cloudLoaded.current = true; setReady(true); }).catch(() => { if (!tournamentId) { localStorage.removeItem("lagata-current-tournament"); setShareId(""); setEditToken(""); } setReady(true); });
    } else { const saved = localStorage.getItem("fc-night-tournament"); if (saved) try { const parsed = JSON.parse(saved); setData({ ...parsed, format: parsed.format || "league", homeAndAway: Boolean(parsed.homeAndAway) }); } catch {} setReady(true); }
  }, []);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/tournament`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ tournament: data }) })
      ...
      .then((result) => { localStorage.setItem("lagata-current-tournament", result.id); localStorage.setItem(`lagata-edit-${result.id}`, result.editToken); localStorage.removeItem("fc-night-tournament"); setShareId(result.id); setEditToken(result.editToken); cloudLoaded.current = true; setShareState("idle"); })
  ...
  function regenerate() { const next = { name: draft.name.trim() || "Friday Night League", format: draft.format, homeAndAway: draft.format === "league" && draft.homeAndAway, players: draft.players, matches: makeFixtures(draft.players, draft.format, draft.homeAndAway) }; setData(next); setRound(1); setShowSetup(false); setTab("matches"); }
