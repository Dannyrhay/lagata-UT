...
function writePendingSync(id: string, tournament: Tournament, baseVersion: string) { localStorage.setItem(pendingSyncKey(id), JSON.stringify({ tournament, baseVersion, queuedAt: new Date().toISOString() } satisfies PendingSync)); }
function clearPendingSync(id: string) { localStorage.removeItem(pendingSyncKey(id)); }
function syncPayload(value: Tournament) { return JSON.stringify(value); }
...
export default function Home() {
  ...
  const [syncStatus, setSyncStatus] = useState<SyncStatus>("saved"); const [lastSyncedAt, setLastSyncedAt] = useState<Date | null>(null); const [syncConflict, setSyncConflict] = useState<SyncConflict | null>(null); const [syncWake, setSyncWake] = useState(0);
  ...
  useEffect(() => { const wake = () => { if (document.visibilityState === "visible") setSyncWake((value) => value + 1); }; document.addEventListener("visibilitychange", wake); window.addEventListener("pageshow", wake); return () => { document.removeEventListener("visibilitychange", wake); window.removeEventListener("pageshow", wake); }; }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      syncChain.current = syncChain.current.then(async () => {
        ...
        const latestResponse = await fetch(`${API_BASE}/api/tournament?id=${encodeURIComponent(shareId)}`); if (!latestResponse.ok) throw new Error("load");
        ...
        const response = await fetch(`${API_BASE}/api/tournament?id=${encodeURIComponent(shareId)}`, { method: "PUT", headers: { "content-type": "application/json", "x-edit-token": editToken, "x-base-version": serverVersion.current }, body: JSON.stringify({ tournament: value }) }); const result = await response.json().catch(() => null); if (response.status === 409 && result?.tournament) { const conflictTournament = normaliseTournament(result.tournament); writePendingSync(shareId, value, serverVersion.current); syncBlocked.current = true; setSyncConflict({ tournament: conflictTournament, version: result.version || result.updatedAt || "" }); setSyncStatus("conflict"); return; } if (!response.ok) throw new Error("save");
        ...
        serverVersion.current = confirmation?.version || result?.version || remoteVersion || serverVersion.current; writeServerVersion(shareId, serverVersion.current); lastSyncedPayload.current = syncPayload(value); clearPendingSync(shareId); rememberTournament(shareId, value.name); setSyncStatus("saved"); setLastSyncedAt(new Date());
      }).catch(() => { writePendingSync(shareId, value, serverVersion.current); setSyncStatus(navigator.onLine === false ? "queued" : "error"); });
    }, 650);
  ...
  async function openTournament(id: string) { const token = localStorage.getItem(`lagata-edit-${id}`) || ""; try { const response = await fetch(`${API_BASE}/api/tournament?id=${encodeURIComponent(id)}`); if (!response.ok) throw new Error(); const result = await response.json(); const remote = normaliseTournament(result.tournament); const pending = token ? readPendingSync(id) : null; const selected = pending?.tournament || remote; setData(selected); setShareId(id); setEditToken(token); serverVersion.current = pending?.baseVersion || result.version || result.updatedAt || ""; writeServerVersion(id, result.version || result.updatedAt || ""); lastSyncedPayload.current = syncPayload(remote); syncBlocked.current = false; setSyncConflict(null); setSyncStatus(pending ? "queued" : "saved"); if (!pending) setLastSyncedAt(new Date()); cloudLoaded.current = true; localStorage.setItem("lagata-current-tournament", id); history.replaceState({}, "", `${location.pathname}?t=${id}`); setRound(1); setTab("matches"); setShowDashboard(false); } catch { setShareState("error"); } }
  ...
  async function connectPwaTournament(value: string) {
    ...
    setData(normalised); setShareId(id); setEditToken(token); serverVersion.current = result.version || result.updatedAt || ""; writeServerVersion(id, serverVersion.current); lastSyncedPayload.current = syncPayload(normalised); syncBlocked.current = false; setSyncConflict(null); setSyncStatus("saved"); setLastSyncedAt(new Date()); cloudLoaded.current = true; creatingCloud.current = false; setNeedsPwaConnection(false); setRound(1); setTab("matches"); history.replaceState({}, "", `${location.pathname}?t=${id}`);
  ...
  function retrySync() { if (!shareId || !editToken) return; syncBlocked.current = false; setSyncStatus(pwa.isOnline ? "saving" : "queued"); setSyncWake((value) => value + 1); }
  const syncLabel = syncStatus === "saving" ? "Syncing changes…" : syncStatus === "offline" ? "Offline · cloud copy unchanged" : syncStatus === "queued" ? "Offline · changes queued" : syncStatus === "conflict" ? "Sync paused · review needed" : syncStatus === "error" ? "Sync failed · changes kept" : "All changes saved";
  const syncDetail = lastSyncedAt ? `Last synced ${lastSyncedAt.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Cloud protection active";
  ...
  return <main>
    ...
    {shareId && editToken && <div className={`syncBar ${syncStatus}`} role="status" aria-live="polite"><span aria-hidden="true" /><div><b>{syncLabel}</b><small>{syncDetail}</small></div>{syncStatus === "error" && <button onClick={retrySync}>Retry</button>}{syncStatus === "conflict" && <button onClick={() => setSyncConflict((current) => current ? { ...current } : current)}>Review</button>}</div>}
